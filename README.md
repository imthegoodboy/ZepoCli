...
- Cart parsing skips delivery-address blocks with custom saved-address labels by using structural address detail, not a fixed address-label list or service-city allow-list. It also skips inactive saved-for-later sections, unavailable item sections, checkout/payment panels, promo gift rows, offer/upsell rows, and membership rows instead of counting those rows as active cart items.
