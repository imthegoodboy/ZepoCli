...
describe("package CLI contract", () => {
  ...
  it("keeps installed package verification checking shipped README guidance", () => {
    ...
    expect(verifier).toContain('checkout.command: \\"zepo --visible checkout\\"');
    expect(verifier).toContain('checkout.waitCommand: \\"zepo --visible checkout --wait\\"');
    expect(verifier).toContain('checkout.paymentLink: \\"https://www.zepto.com/?cart=open\\"');
    expect(verifier).toContain('checkout.paymentLinkSession: \\"user_zepto_session_required\\"');
    expect(verifier).toContain("When `verify:live` reaches `checkout_manual_action_required`");
    expect(verifier).toContain("That console guidance is not payment proof or order proof");
    expect(verifier).toContain("without guessing a `/cart` route");
    ...
    expect(verifier).toContain('paymentHandoffUrl: \\"https://www.zepto.com/?cart=open\\"');
    expect(verifier).toContain('paymentLink: \\"https://www.zepto.com/?cart=open\\"');
    expect(verifier).toContain('paymentLinkSession: \\"user_zepto_session_required\\"');
    expect(verifier).toContain('handoffSurface: \\"visible_zepto_browser\\"');
    expect(verifier).toContain("expected installed checkout payment handoff URL to match the Zepto handoff URL marker");
    expect(verifier).toContain("expected installed checkout payment link to match the Zepto handoff URL marker");
    expect(verifier).toContain("expected installed checkout payment link to require the user's Zepto session");
