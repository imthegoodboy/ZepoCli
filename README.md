...
`verify:live:report` does not contact Zepto or prove a fresh run happened. It checks the saved report contract: package version, `ok`, sanitized `generatedAt` plus data/report path metadata, accepted report schema, complete boolean capability summaries, redacted step command contract, consistent step `exitCode`/`ok`/`summary`/`error` fields, `ok` reports containing only passing known workflow steps, unique workflow step names, `attempted`/`coverage` consistency with `steps`, sensitive-looking key/value redaction, requested coverage, `missingCoverage`, and required step summaries for browser preflight, live session, checkout handoff, and requested workflows.
