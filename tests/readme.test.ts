...
describe("README package guidance", () => {
  ...
  it("documents the agent preflight and checkout handoff contract", () => {
    ...
    expect(readme).toContain("checkout/payment panels, promo gift panels, offer/upsell panels, membership rows, and image alt/accessibility text");
    ...
    expect(readme).toContain("checkout/payment panels, promo gift rows, offer/upsell rows, and membership rows instead of counting those rows as active cart items");
    ...
    expect(readme).toContain("skips inactive saved-for-later sections, unavailable item sections, checkout/payment panels, promo gift rows, offer/upsell rows, and membership rows");
