...
`zepo --visible checkout --json` returns structured handoff evidence immediately instead of waiting for an Enter prompt, so live verification can parse the checkout boundary without hanging. Use `--checkout-wait` only when a human is ready to complete Zepto-side checkout/payment in the visible browser before the runner continues to tracking. Use `--checkout-remove-limit-items` only for the explicit Zepto item-limit warning removal before that checkout handoff; it does not make checkout/payment autonomous. Wait mode re-checks the visible page after Enter; if a human-clicked manual continuation reaches a real checkout/payment handoff surface, the checkout step may return `checkout_handoff_returned`. Checkout JSON and sanitized checkout report evidence must include fixed `handoffUrl: "https://www.zepto.com/?cart=open"` and `handoffSurface: "visible_zepto_browser"` markers; they identify the Zepto browser handoff surface only and are not payment/order proof. `status: "checkout_manual_action_required"` proves only a readable non-empty cart plus safe manual Zepto continuation. The live runner reports it as `live_verification_incomplete` and may include sanitized `manualEvidence` with `humanActionRequired`, `automationBoundary`, `handoffUrl`, and `handoffSurface`; it is intentionally not accepted as checkout handoff coverage or production-scope readiness evidence.
...
- Checkout coverage requires `status: "checkout_handoff_returned"`; `status: "checkout_manual_action_required"` and any `manualEvidence` must keep coverage missing because a human still needs to continue inside Zepto manually.
