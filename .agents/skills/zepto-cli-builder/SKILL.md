...
After a human-controlled live run, `npm --silent run verify:live:report -- <report-path>` should validate the saved report contract before agents treat it as acceptance evidence. The report validator does not contact Zepto or prove a fresh run happened, and it must reject reports with fields outside the accepted schema, step command strings outside the redacted command contract, `attempted` or `coverage` summaries that do not match the saved `steps` array, or keys/values containing sensitive-looking local paths, phone/order/payment/verification values, or npm-token-shaped values.
