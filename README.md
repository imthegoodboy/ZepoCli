...
- Cart parsing skips delivery-address blocks with custom saved-address labels by using structural address detail, not a fixed address-label list or service-city allow-list. It also skips inactive saved-for-later and unavailable item sections instead of counting those rows as active cart items.
