...
- Cart navigation and cart remove/clear automation use visible, enabled cart controls only. Tagged remove/decrease controls are revalidated against the current cart row before click, and `zepo remove <query>` requires that row to still match the requested item.
