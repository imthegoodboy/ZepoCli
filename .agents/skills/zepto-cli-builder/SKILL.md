...
After a human-controlled live run, `npm --silent run verify:live:report -- <report-path>` should validate the saved report contract before agents treat it as acceptance evidence. The report validator does not contact Zepto or prove a fresh run happened, and it must reject reports with missing or malformed top-level report metadata, incomplete or non-boolean capability summaries, fields outside the accepted schema, `ok` reports containing failed, manual, internal, unknown, or duplicate workflow steps, step command strings outside the redacted command contract, inconsistent step `exitCode`/`ok`/`summary`/`error` fields, `attempted` or `coverage` summaries that do not match the saved `steps` array, or keys/values containing sensitive-looking local paths, phone/order/payment/verification values, or npm-token-shaped values.
