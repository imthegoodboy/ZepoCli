...
- After a human-controlled live run, use `npm --silent run verify:live:report -- <report-path>` to check report acceptability. The report validator does not contact Zepto or prove a fresh run happened; it checks package version, `ok`, accepted report schema, redacted step command contract, `attempted`/`coverage` consistency with `steps`, sensitive-looking key/value redaction, requested coverage, `missingCoverage`, and required step summaries before agents treat a report as evidence.
