...
For final production-scope proof, the human must finish any Zepto-side checkout continuation in the visible browser before the runner can move on to `track`. When checkout wait is requested, the runner prints the fixed `Payment link: https://www.zepto.com/?cart=open` and `Payment link session: user_zepto_session_required` before launching the waiting checkout command, so a later prompt timeout still leaves the Zepto-owned session link in the console. If Zepto shows an amount-bearing cart/payment button, review the cart in Zepto, open or continue from `paymentLink` when a link handoff is needed, use only Zepto's visible checkout/payment UI for any payment, OTP, PIN, age, prescription, or delivery-verification step, then return to the terminal and press Enter only after the Zepto-side continuation is complete. ZepoCli will not ask for those values, store them, or click final payment/order controls; without that human continuation the saved report should remain at `checkout_manual_action_required` and must fail production-scope acceptance.
