...
- Checkout coverage requires `status: "checkout_handoff_returned"`; `status: "checkout_manual_action_required"` and any `manualEvidence` must keep coverage missing because the user still needs to click Zepto's visible payment control manually.
