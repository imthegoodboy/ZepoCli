...
- Address automation may open the add-address UI only; never click current/device/precise-location sharing, browser location-access/GPS permission, payment-method/payment controls, or final address-confirmation controls. Reject address manager/add-address controls and saved-address extraction when any visible or accessible label contains location-consent, final address-confirmation, or payment-method/payment text, even if another label looks safe.
