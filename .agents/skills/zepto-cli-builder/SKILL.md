...
4. Automation behavior.
   ...
   - Installed-package commands should run browser automation in background/headless mode by default. Show the browser only for explicit `--visible` runs or when a human-only login, address-add, or checkout handoff is required. Background/headless mode is for explicit user-requested CLI commands only, not autonomous ordering or payment.
