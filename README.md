...
8. After `zepo --visible checkout --json`, inspect `status`, `cartPrecondition`, `paymentStatus`, and `orderPlacement`. JSON checkout returns handoff evidence immediately for agents instead of waiting for a prompt; use human text mode (`zepo --visible checkout`) or explicit JSON wait mode (`zepo --visible checkout --json --wait`) when the browser must stay open for Zepto-side payment, then run `zepo track --json` only after payment is completed. Wait mode re-checks the visible page after the human presses Enter, so a human-clicked manual continuation can still report `checkout_handoff_returned` if Zepto exposes a real checkout/payment handoff surface. `status: "checkout_manual_action_required"` means Zepto showed only a manual payment control; it is not payment, order placement, or production-scope checkout handoff coverage, and live verification reports it with `live_verification_incomplete` plus sanitized `manualEvidence`.
...
- `zepo cart` returns an empty cart only when Zepto shows explicit empty-cart copy without non-empty cart signals such as item counts, bill/total, checkout, quantity, or remove controls. If the cart page opens but items are unreadable, the CLI fails instead of treating the cart as empty. Plain `zepo cart` and plain `zepo --visible checkout` are read-only when Zepto shows item-limit warnings; use `zepo cart --remove-limit-items` or `zepo --visible checkout --remove-limit-items` only when you explicitly want ZepoCli to click Zepto's visible `Remove Items` action, handle repeated item-limit warnings in a bounded sequence, and reread the cart. `zepo clear` may resolve the same warning because it is already an explicit request to remove cart contents.
