...
describe("CLI command smokes", () => {
  ...
  it("prints machine-readable parser errors when malformed json flag values are used", async () => {
    const result = await runCli(["--json=true", "status", "--bad-option"]);
    ...
    expect(payload.error.type).toBe("invalid_input");
    expect(payload.error.code).toBe("invalid_input");
    expect(payload.error.message).toBe("error: unknown option '--json=true'");
    expect(payload.error.exitCode).toBe(1);
  }, CLI_TEST_TIMEOUT_MS);
