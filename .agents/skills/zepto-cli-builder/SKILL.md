...
5. CLI output behavior.
   ...
   - Human and JSON failure `message`, `hint`, and issue text should redact sensitive-looking order-id, phone, OTP/PIN/CVV, payment-number, payment-handle, auth/session/token URL parameters, and local-path values before printing, including URL/query-string encoded forms of those values. It should also redact npm-token-shaped values.
   - Persistent runtime log object values, Error messages/stacks, and message strings should use the same sensitive-looking order-id, phone, OTP/PIN/CVV, payment-number, payment-handle, auth/session/token URL-parameter, and local-path redaction rules as terminal errors, including URL/query-string encoded forms of those values. They should also redact npm-token-shaped values.
