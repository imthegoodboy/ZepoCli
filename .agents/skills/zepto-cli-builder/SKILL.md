...
4. Automation behavior.
   ...
   - Zepto Terms of Use version 1.4 were checked on 2026-05-30 at https://www.zepto.com/s/terms-of-service and show "Last updated: 1 st November 2025".
   - Zepto Privacy Notice version 1.1 was checked on 2026-05-30 at https://staticweb.zepto.com/privacy-policy/ and shows "Last updated: 17th June 2025"; it treats passwords and payment instrument details as sensitive personal information, describes payment processing through payment gateways, and tells users to avoid sharing login credentials, passwords, or OTPs.
...
5. CLI output behavior.
   ...
   - Human and JSON failure `message`, `hint`, and issue text should redact sensitive-looking order-id, phone, OTP/PIN/CVV, payment-number, payment-handle, auth/session/token URL parameters, and local-path values before printing, including URL/query-string encoded forms of those values.
   - Persistent runtime log object values, Error messages/stacks, and message strings should use the same sensitive-looking order-id, phone, OTP/PIN/CVV, payment-number, payment-handle, auth/session/token URL-parameter, and local-path redaction rules as terminal errors, including URL/query-string encoded forms of those values.
