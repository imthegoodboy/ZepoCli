...
4. Automation behavior.
   ...
   - Cart remove/clear should target enabled controls inside likely active product item rows only; ignore coupon, bill summary, fee, discount, total, inactive saved/unavailable item sections, and order-history/action rows even if they expose remove/decrease controls. Tagged remove/decrease controls must reject mixed unsafe visible/accessibility labels such as coupon, address, checkout, payment-method/payment, inactive saved/unavailable item actions, or order actions including order summary, tracking, reorder, cancellation, refund, support, invoice, receipt, and rating, must be revalidated against the current cart row before click, including after any scroll into view, and `zepo remove <query>` must still match the requested item.
