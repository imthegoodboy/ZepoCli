...
Checkout handoff controls are rejected if any visible or accessible label contains payment-method, final-payment, or final-order text, even when another label looks like a safe checkout handoff.
...
Product extraction ignores delivery-speed and promo UI badges such as `10 MINS`, `Super Saver`, and `Lowest Price` when choosing product names.
...
- `zepo history` returns an empty list only when Zepto shows explicit empty-history copy without unreadable order signals such as reorder, order summary, track order, ETA, or status text. If the orders page opens but order cards are unreadable, the CLI fails instead of treating history as empty.
