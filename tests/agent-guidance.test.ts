...
describe("agent guidance", () => {
  ...
  it("keeps future-agent address and payment safety boundaries explicit", () => {
    for (const guidance of [agentReadme, builderSkill]) {
      ...
      expect(guidance).toContain("checkout/payment panels, promo gift panels, offer/upsell panels, membership rows");
      ...
      expect(guidance).toContain("inactive saved-for-later sections, unavailable item sections, checkout/payment panels, promo gift rows, offer/upsell rows, and membership rows");
      ...
      expect(guidance).toContain("2026-06-03");
