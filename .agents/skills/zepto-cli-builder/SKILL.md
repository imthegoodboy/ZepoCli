...
3. Payment boundary.
   ...
   - Address automation may open the add-address UI only; never click current/device/precise-location sharing, browser location-access/GPS permission, or final address-confirmation controls. Reject address manager/add-address controls and saved-address extraction when visible or accessible copy contains location-consent or final address-confirmation text, even if another label looks safe.
...
4. Automation behavior.
   ...
   - Product/cart/order parsers must ignore UI badges, generic image alt text, fee rows, and summary rows instead of treating them as real user data. Product extraction should ignore delivery-speed and promo badges such as `10 MINS`, `Super Saver`, and `Lowest Price` when choosing product names.
   ...
   - Address add should click only visible, enabled, explicit add-address controls; never click current/device/precise-location sharing, browser location-access/GPS permission, save/confirm address, selected-address copy, or mixed-label controls where any label is unsafe. Saved-address extraction should reject location-consent and final address-confirmation copy even when it contains city or pincode text.
