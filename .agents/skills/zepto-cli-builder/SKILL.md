...
3. Payment boundary.
   ...
   - Address automation may open the add-address UI only; never click current/device/precise-location sharing, browser location-access/GPS permission, payment-method/payment controls, or final address-confirmation controls. Reject address manager/add-address controls and saved-address extraction when visible or accessible copy contains location-consent, final address-confirmation, or payment-method/payment text, even if another label looks safe.
...
4. Automation behavior.
   ...
   - Address add should click only visible, enabled, explicit add-address controls; never click current/device/precise-location sharing, browser location-access/GPS permission, payment-method/payment controls, save/confirm address, selected-address copy, or mixed-label controls where any label is unsafe. Saved-address extraction should reject location-consent, final address-confirmation, and payment-method/payment copy even when it contains city or pincode text.
