...
- `cart`, `remove`, `clear`, `checkout`, `track`, `history`, and `reorder` satisfy their named live report contracts when requested. `history` coverage requires at least one readable order record with status or ETA; an empty `zepo history` result is valid CLI output only, not order-history live-report coverage.
