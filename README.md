...
Product extraction ignores text that Zepto exposes as product-card control labels when choosing product names, and ADD-control mapping rejects mixed controls whose labels are not explicit product-add labels. This keeps unavailable or alternate-action controls from being treated as product names or safe ADD buttons without baking those labels into the parser.
