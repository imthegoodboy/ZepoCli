...
- Checkout coverage requires `status: "checkout_handoff_returned"` plus the fixed handoff/lifecycle markers including `browserOpenAfterReturn: false` and `checkoutWaitCompleted`; production-scope checkout coverage additionally requires `checkoutWaitCompleted: true`. `status: "checkout_manual_action_required"` and any `manualEvidence` must keep coverage missing because a human still needs to continue inside Zepto manually.
